...
1. В блоке **«Подключение по QR»** выберите нужную сеть:
   - **VPN** — если телефон подключён к VPN и вы вне дома
...
| ----------------- | -------------------------------------------------------------------------------------------- |
| **Сервер**        | Адрес из панели, например `http://198.51.100.10:8080` (VPN) или `http://192.0.2.50:8080` (дома) |

> В примерах ниже — **тестовые IP** из диапазонов [RFC 5737](https://datatracker.ietf.org/doc/html/rfc5737). В приложении указывайте адрес из вашей панели управления.
...
| --------------------- | ----------------------------------------------------- |
| Дома, телефон в Wi‑Fi | `http://192.0.2.x:8080` (Ethernet / Wi‑Fi в панели) |
| Через Amnezia VPN     | `http://198.51.100.x:8080` (VPN в панели)           |
...
```json
{ "server": "http://198.51.100.10:8080", "token": "ваш-token" }
```
...
```json
{ "server": "http://192.0.2.50:8080", "token": "change-me-to-a-secure-token" }
```
...
```swift
let baseURL = URL(string: "http://192.0.2.100:8080")!
